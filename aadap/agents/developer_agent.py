"""
AADAP — Developer Agent (Databricks Python)
===============================================
Code-generation agent for Databricks Python / PySpark tasks.
...
class DeveloperAgent(BaseAgent):
    """
    Databricks Python / PySpark code generation agent.

    Generates production-quality code based on task descriptions.
    Self-corrects on schema violations up to INV-03 bound.
    """
    ...
    async def _do_execute(self, context: AgentContext) -> AgentResult:
        """
        Generate Databricks Python code for the given task.
        ...
        for attempt in range(1, MAX_SELF_CORRECTIONS + 1):
            ...
            try:
                result = validate_output(
                    response.content, DEVELOPER_CODE_SCHEMA
                )
                logger.info(
                    "developer.success",
                    agent_id=self._agent_id,
